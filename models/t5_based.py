...
class T5EncoderForSequenceClassification(T5EncoderModel):
    ...
    def __init__(self, config: T5ConfigForSequenceClassification):
        ...
        self.rating_classifier = Linear(in_features=self.config.hidden_size, out_features=self.num_labels)
    ...
    def forward(self, input_ids=None,
        ...
        last_hidden_states = encoder_outputs[0]  # (batch_size, hidden_size)

        # avg the embeddings of all tokens in the sequence to get an embedding for the sequence
        avg_embeddings = torch.mean(last_hidden_states, dim=1)
        ...
        if not return_dict:
            output = (rating_logits,) + encoder_outputs[2:]
...
class T5WithBehaviourRatingPredictorConfig(T5Config):
...
class T5EncoderWithBehaviourRatingPredictor(T5EncoderModel):

    def __init__(self, config: T5WithBehaviourRatingPredictorConfig):
    ...
    def forward(self, input_ids=None,
        ...
        last_hidden_states = outputs[0]  # (batch_size, hidden_size)
